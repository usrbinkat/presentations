<!--
  SpeakerBio.vue — Speaker introduction card.
  Horizontal layout with avatar, name, title, links, and optional bio text.

  Usage in slides.md:
    <SpeakerBio name="usrbinkat" title="Platform Engineer — Braincraft" />

    <SpeakerBio
      name="Kat Morgan"
      title="Principal Engineer"
      avatarUrl="https://github.com/usrbinkat.png"
      :links="['@usrbinkat', 'github.com/usrbinkat']"
    >
      Building reproducible infrastructure with Nix and Kubernetes.
    </SpeakerBio>
-->
<script setup lang="ts">
const {
  name,
  title = '',
  avatarUrl = '',
  links = [],
} = defineProps<{
  name: string
  title?: string
  avatarUrl?: string
  links?: string[]
}>()
</script>

<template>
  <div class="speaker-bio">
    <div class="speaker-avatar-wrap">
      <img
        v-if="avatarUrl"
        :src="avatarUrl"
        :alt="name"
        class="speaker-avatar"
      >
      <div
        v-else
        class="speaker-avatar speaker-avatar-placeholder"
        aria-hidden="true"
      >
        {{ name.charAt(0).toUpperCase() }}
      </div>
    </div>
    <div class="speaker-info">
      <div class="speaker-name">
        {{ name }}
      </div>
      <div v-if="title" class="speaker-title">
        {{ title }}
      </div>
      <div v-if="links.length" class="speaker-links">
        <a
          v-for="(link, i) in links"
          :key="i"
          :href="link.startsWith('http') ? link : `https://${link}`"
          target="_blank"
          rel="noopener noreferrer"
          class="speaker-link"
        >{{ link }}</a>
      </div>
      <div v-if="$slots.default" class="speaker-bio-text">
        <slot />
      </div>
    </div>
  </div>
</template>

<style scoped>
.speaker-bio {
  container-type: inline-size;
  display: flex;
  align-items: center;
  gap: var(--aurora-space-6);
  padding: var(--aurora-space-6);
  background: var(--scheme-bg, var(--aurora-cream-100));
  border-radius: var(--aurora-radius-xl);
  box-shadow: var(--scheme-shadow, var(--aurora-shadow-sm));
}

.speaker-avatar {
  width: var(--aurora-space-20);
  height: var(--aurora-space-20);
  border-radius: var(--aurora-radius-full);
  object-fit: cover;
  border: var(--aurora-border-medium) solid var(--scheme-border, var(--aurora-lavender-300));
}

.speaker-avatar-placeholder {
  display: flex;
  align-items: center;
  justify-content: center;
  background: var(--scheme-highlight, var(--aurora-lavender-300));
  color: var(--scheme-heading, var(--aurora-slate-700));
  font-size: var(--aurora-text-2xl);
  font-weight: var(--aurora-font-bold);
}

.speaker-info {
  display: flex;
  flex-direction: column;
  gap: var(--aurora-space-1);
}

.speaker-name {
  font-size: var(--aurora-text-xl);
  font-weight: var(--aurora-font-semibold);
  color: var(--scheme-text, var(--aurora-slate-800));
}

.speaker-title {
  font-size: var(--aurora-text-sm);
  color: var(--scheme-text-secondary, var(--aurora-slate-500));
  font-weight: var(--aurora-font-medium);
}

.speaker-links {
  display: flex;
  gap: var(--aurora-space-4);
  margin-top: var(--aurora-space-1);
}

.speaker-link {
  font-size: var(--aurora-text-xs);
  font-family: var(--aurora-font-mono);
  color: var(--scheme-accent, var(--aurora-lavender-500));
  text-decoration: none;
  border-bottom: 1px solid transparent;
  transition: border-color var(--aurora-duration-fast) var(--aurora-ease-out);
}

.speaker-link:hover {
  border-bottom-color: currentColor;
}

.speaker-bio-text {
  margin-top: var(--aurora-space-2);
  font-size: var(--aurora-text-sm);
  color: var(--scheme-text-secondary, var(--aurora-slate-600));
  line-height: var(--aurora-leading-relaxed);
}

@container (max-width: 350px) {
  .speaker-bio {
    flex-direction: column;
    text-align: center;
  }

  .speaker-avatar-wrap {
    display: flex;
    justify-content: center;
  }

  .speaker-avatar {
    width: var(--aurora-space-16);
    height: var(--aurora-space-16);
  }

  .speaker-links {
    justify-content: center;
  }
}
</style>
