...
All layouts accept `color` and `class` props. The `color` prop selects a color scheme (default varies per layout).
...
Title slide with large heading. Default color: `lavender`. Also accepts `background` prop for background images.
...
Title spanning full width above two columns. Default color: `cream`. Accepts `columns` prop for custom `grid-template-columns` (default `1fr 1fr`). Uses default slot for title, `left` and `right` named slots for columns.
...
Title on the left side, content on the right. Default color: `cream`. Accepts `titleWidth` prop (default `1fr`, combined with `2fr` for content). Uses `title` named slot for left side, default slot for right content.
...
Styled blockquote layout. Default color: `lavender`. Vertically centered, large italic text with accent left border. Best used with `> blockquote` markdown syntax.
...
Big impact number or statistic. Default color: `mint`. Centered content with oversized heading (`text-5xl`) and smaller supporting text.
...
Bold assertion text. Default color: `lavender`. Centered both ways, `text-4xl`, bold, with `text-wrap: balance`.
...
Speaker bio with optional image. Default color: `cream`. Accepts `imageSrc` prop for speaker photo (resolved via `resolveAssetUrl`). 2-column grid when image is provided, single column otherwise.
...
Image with caption. Default color: `cream`. Requires `figureUrl` prop. Optional `figureCaption` prop for caption text. Default slot available for additional content below.
...
Closing/thank-you slide. Default color: `slate`. Grid centered like `center` layout but with slate scheme for a strong closing.
...
Each scheme provides `--scheme-bg`, `--scheme-text`, `--scheme-heading`, `--scheme-accent`, and `--scheme-muted` variables.
...
Activated via `html.dark` class (Slidev toggle). Each color scheme has dark variants defined in `styles/dark-mode.css`.
